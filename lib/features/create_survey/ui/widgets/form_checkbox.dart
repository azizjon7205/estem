...
import 'package:estem/shared/models/questions/question_checkbox.dart';
...
class _FormCheckboxState extends State<FormCheckbox> {
  ...
  void onSwitchValueChanged(bool value) {
    ...
    setState(() {
      isRequired = value;
      query = query.copyWith(isRequired: value) as CheckBoxQuestion;
    });
  }

  void onHasOtherValueChanged(bool value) {
    setState(() {
      query = query.copyWith(hasOther: value) as CheckBoxQuestion;
    });
  ...
  void onPickImage() async {
    ...
    try {
      ...
      setState(() {
        query = query.copyWith(image: base64Image) as CheckBoxQuestion;
  ...
  void onRemoveOption(dynamic id) {
    setState(() {
      query.options.removeWhere((element) => element.id == id);
    });
    widget.onQuestionChanged(query);
  }
      ...
      child: Column(
        ...
        children: [
          AppTextField(
            ...
            onChanged: (value) {
              query = query.copyWith(question: value) as CheckBoxQuestion;
              widget.onQuestionChanged(query);
            },
          ),
          if (_imageBytes != null) ...[
