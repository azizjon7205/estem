...
import 'package:estem/shared/models/questions/question_smile.dart';
...
class FormSmile extends StatefulWidget {
  const FormSmile(
      {super.key,
  ...
  @override
  State<FormSmile> createState() => _FormSmileState();
}

class _FormSmileState extends State<FormSmile> {
  late SmileQuestion query;
  ...
  void onSwitchValueChanged(bool value) {
    setState(() {
      query = query.copyWith(isRequired: value) as SmileQuestion;
    });
  ...
  void onPickImage() async {
    try {
      ...
      setState(() {
        query = query.copyWith(image: base64Image) as SmileQuestion;
  ...
  @override
  void didUpdateWidget(covariant FormSmile oldWidget) {
      ...
      child: Column(
        ...
        children: [
          AppTextField(
            ...
            onChanged: (value) {
              query = query.copyWith(question: value) as SmileQuestion;
              widget.onQuestionChanged(query);
            },
          ),
          if (_imageBytes != null) ...[
          ...
          ...query.ratingLabels.entries.map(
            (entry) => FormItemOption(
              ...
              leading: SvgPicture.asset(
                  getSmileIcons()[entry.key]?.replaceAll('_pale', '') ??
                      'assets/faces/ic_face_neutral_pale.svg'),
              hint: 'question.strings.${getSmileStrings()[entry.key] ?? 'neutral'}'.tr(),
